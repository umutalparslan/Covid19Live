...
class PNBarChart: UIView {
    ...
    var yChartLabelWidth: CGFloat = 50
    var chartMargin: CGFloat = 30
    var showLabel = true
    ...
    var labelTextFont = UIFont(name: "Avenir-Medium", size: 9.0)
    var xLabelSkip = 1
    var yLabelSum = 7
...
//             Add Y Labels
            ...
            for index in 0..<self.yLabelSum {
                ...
                let label = PNChartLabel(frame: CGRect(x: self.chartMargin / 3, y: yLabelSectionHeight * CGFloat(index) + self.chartMargin - self.yLabelHeight / 2.0, width: self.yChartLabelWidth, height: self.yLabelHeight))
                label.font = self.labelTextFont
                label.textColor = self.labelTextColor
